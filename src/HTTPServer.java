...
public class HTTPServer {
	...
	public class Request {
		...
		protected String remoteAddress; // address of client that connected to us
	...
	protected void handleConnection(Socket sock) throws IOException {
		...
		OutputStream out = new BufferedOutputStream(sock.getOutputStream(), 4096);
		Request req;
		Response resp;
		do {
			...
			try {
				req = new Request(in);
				req.remoteAddress = sock.getInetAddress().getHostAddress();
	...
	public static void main(final String[] args) {
		try {
			...
			ContextHandler botHandler = new ContextHandler() {
				public int serve(Request req, Response resp) throws IOException {
					String file = req.getPath().substring("/bhbot/".length());
					String cookie = req.getHeaders().get("Cookie");
					//System.out.println("DEBUG: COOKIE=<" + cookie + ">");
					if (cookie != null && !cookie.equals("login=secret"))
						cookie = null; // reset cookie, since it uses wrong/expired credentials
					
					if (file.equals("getaccesstoken")) {
						String user = req.getParams().get("user");
						String serial = req.getParams().get("serial");
						
						System.out.println("ACCESS: Request from <" + req.remoteAddress + ">, user=<" + user + ">, serial=<" + serial + ">.");
						
						if (user == null) {
							System.out.println("ERROR: Invalid username for access token: <null>. Sender: " + req.remoteAddress);
							resp.send(200, "DENIED"); // we'll just deny access to the user
						}
						String token = server.generateAccessToken(1000 * 60 * 60 * 24 * 14 /*two weeks*/, serial);
						resp.send(200, token);
						System.out.println("ACCESS: Token sent: <" + token + ">");
						server.logAccessRequest(req.remoteAddress); // we log it in order to have some control over it (like to figure out who is using the bot)
						
						return 0;
					}
					
					if (file.equals("try_login") && req.method.equals("POST")) {
						String username = req.getParams().get("uname");
						String password = req.getParams().get("psw");
						
						System.out.println("*** User attempted to log in. Credentials:");
						...
						if ("test".equals(username) && "test".equals(password)) {
							...
							resp.getHeaders().add("Set-Cookie", "login=secret; " + server.constructCookieExpirationString());
							return server.sendPage("/bhbot/login_ok.html", req, resp);
						} else {
							// NOT OK, wrong credentials!
							System.out.println("*** CREDENTIALS: WRONG!");
							return server.sendPage("/index.html", req, resp);
						}
					}
					
					if (cookie == null) {
						// user need to log in first!
						System.out.println("*** User needs to log in first! Sending the login page...");
						return server.sendLoginPage(req, resp);
					}
	            	/*
                	System.out.println("DEBUG: --------------------------------- REQ ---------------------------------");
                	System.out.println("DEBUG: baseUrl=<" + req.baseURL + ">");
                	System.out.println("DEBUG: baseUrl.file=<" + req.baseURL.getFile() + ">");
                	System.out.println("DEBUG: path=<" + req.getPath() + ">");
                	System.out.println("DEBUG: method=<" + req.getMethod() + ">");
                	//System.out.println("DEBUG: body=<" + req.getBody() + ">");
                	System.out.println("DEBUG: params=<" + req.getParams() + ">");
                	System.out.println("DEBUG: URI=<" + req.getURI() + ">");
                	Iterator<Header> it = req.getHeaders().iterator();
                	while (it.hasNext())
                		System.out.println("DEBUG: headers[...]=<" + it.next() + ">");
                	*/
					Misc.log("REQ: path=<" + req.getPath() + ">");
					
					// serve the log file:
					if (file.equals("getlog")) {
						int n = 50;
						try {
							n = Integer.parseInt(req.getParams().getOrDefault("n", "" + 50));
						} catch (Exception e) {e.printStackTrace();
						}
						server.serveLogFile(n, req, resp);
						return 0;
					}
					
					// serve the log file:
					if (file.equals("getlogsize")) {
						
						// in Linux:
						String[] command = {"/bin/bash", "./getlogsize.sh"};
					...
					// serve screenshot:
					if (file.equals("getscreenshot")) {
						server.serveScreenshot(req, resp);
						return 0;
					}
					
					// serve settings.ini file:
					if (file.equals("getsettingsfile")) {
						System.out.println("Uploading settins.ini file...");
						File f = new File("../settings.ini");
						serveFileContent(f, req, resp);
						return 0;
					}
					
					// respond to "setsettingsfile":
					// http://www.freeutils.net/source/jlhttp/faq#q_how_do_i_handle_uploaded_files
					if (file.equals("setsettingsfile")) {
						System.out.println("Writing settins.ini file...");
						File f = new File("../settings.ini");
						String comment;
						String filename;
						
						MultipartIterator iter = new MultipartIterator(req);
						while (iter.hasNext()) {
							MultipartIterator.Part part = iter.next();
						...
						resp.send(200, "SETTINGS_UPDATED_OK");
						return 0;
					}
					
					if (file.equals("command")) {
						String cmd = URLDecoder.decode(req.getParams().getOrDefault("cmd", "").replace("+", "%2B"), "UTF-8");
						System.out.println("DEBUG: cmd=" + cmd);
						
						// in Linux:
						//String[] command = {"/bin/bash", "screen -S bhbot -p 0 -X stuff \"" + cmd + "$(printf \\\\r)\""};
						String[] command = {"/bin/bash", "./execcmd.sh", cmd};
						System.out.println("DEBUG: system cmd issued: <" + command[1] + ">");
						ProcessBuilder p = new ProcessBuilder(command);
						Process proc = p.start();
						BufferedReader br = new BufferedReader(new InputStreamReader(proc.getInputStream()));
						String line;
						while ((line = br.readLine()) != null) {
							System.out.println("DEBUG: sys cmd returned: <" + line + ">");
						}
						System.out.println("DEBUG: sys cmd is done.");
						br.close();
						
						resp.send(200, "COMMAND_OK");
						return 0;
					}
					
					// serve "debug" files:
					if (file.equals("debuglist")) {
						System.out.println("Serving debug file list...");
						resp.send(200, listDebugFiles(new File(args[0]).getCanonicalFile().getParentFile().getParentFile()));
						return 0;
					}
					
					// serve "debug" file:
					if (file.equals("fetch")) {
						System.out.println("Fetching debug file...");
						String name = URLDecoder.decode(req.getParams().get("file").replace("+", "%2B"), "UTF-8");
						if (!acceptDebugFileName(name)) {
							resp.sendError(500, "Error: debug file name is malformed: <" + name + ">.");
							return 0;
						}
					...
					return server.serveMainPage(req, resp);
				}
			};
			host.addContext("/bhbot/main.html", botHandler, "GET", "POST");
			host.addContext("/bhbot/try_login", botHandler, "GET", "POST");
			host.addContext("/bhbot/getlog", botHandler, "GET", "POST"); // request bot's log to be sent over
			host.addContext("/bhbot/command", botHandler, "GET", "POST"); // send a command to the bot
			host.addContext("/bhbot/getlogsize", botHandler, "GET", "POST");
			host.addContext("/bhbot/getscreenshot", botHandler, "GET", "POST");
			host.addContext("/bhbot/getaccesstoken", botHandler, "GET", "POST");
			host.addContext("/bhbot/getsettingsfile", botHandler, "GET", "POST");
			host.addContext("/bhbot/setsettingsfile", botHandler, "GET", "POST");
			host.addContext("/bhbot/debuglist", botHandler, "GET", "POST");
			host.addContext("/bhbot/fetch", botHandler, "GET", "POST");
	...
	private int sendLoginPage(Request req, Response resp) throws IOException {
		VirtualHost host = getVirtualHost(null);
		String path = "/bhbot/login.html";
		req.setPath(path);
		return host.getContext("/").getHandlers().get("GET").serve(req, resp);
	}
	
	private int sendPage(String path, Request req, Response resp) throws IOException {
		VirtualHost host = getVirtualHost(null);
		req.setPath(path);
		req.setMethod("GET");
		return host.getContext("/").getHandlers().get("GET").serve(req, resp);
	...
	private String constructCookieExpirationString() {
		// construct cookie expiration date string:
		...
		expdate.setTime(expdate.getTime() + (315360000000l)); // 10 years
	...
	private int serveMainPage(Request req, Response resp) throws IOException {
		VirtualHost host = getVirtualHost(null);
		req.setPath("/bhbot/main.html");
		return host.getContext("/").getHandlers().get("GET").serve(req, resp);
	}
	
	private void serveLogFile(int n, Request req, Response resp) throws IOException {
		// in Linux:
		String[] command = {"/bin/bash", "web_log.sh", "" + n};
		ProcessBuilder p = new ProcessBuilder(command);
		Process proc = p.start();
		BufferedReader br = new BufferedReader(new InputStreamReader(proc.getInputStream()));
		String line;
        ...
        /*
    	// in Windows:
    	List<String> lines = Misc.readTextFile2("E:/eclipse/workspace/BHBot/web/bhbot/output.txt");
    	if (lines.size() > n)
    		while (lines.size() > n)
    			lines.remove(0);
		...
		*/
		...
		resp.send(200, sb.toString());
	}
	
	private void serveScreenshot(Request req, Response resp) throws IOException {
		File file = new File("../shot.png");
		serveFileContent(file, req, resp);
	}
	
	/**
	 * @param validDuration in milliseconds
	 * @return
	 */
	private String generateAccessToken(long validDuration, String serial) {
		final String key = "Some secret key (like a key phrase used for XOR-ing)";
		long validUntil = System.currentTimeMillis() + validDuration;
		return Misc.base64encode(Misc.xorMessage("GRANTED " + validUntil, Misc.xorMessage(key, serial)));
	}
	
	private void logAccessRequest(String ip) {
		FletcherChecksum c = new FletcherChecksum();
		c.add(ip);
		Misc.saveTextFile("./access_logs/" + Math.abs(c.getChecksum()) + ".access", ip + "\r" + System.currentTimeMillis(), true);
	}
	
	private static String listDebugFiles(File dir) {
		// calculate name column width
		int w = 21; // minimum width
		for (String name : dir.list())
			if (name.length() > w)
		...
		Formatter f = new Formatter(Locale.US);
		f.format("<!DOCTYPE html>%n" +
						"<html><head><title>Debug file list</title></head>%n" +
						"<body><h1>Debug file list</h1>%n" +
						"<pre> Name%" + (w - 5) + "s Last modified      Size<hr>",
				"");
		...
		for (File file : files) {
			try {
				...
				String size = file.isDirectory() ? "- " : toSizeApproxString(file.length());
				// properly url-encode the link
				String link = "fetch?file=" + new URI(null, name, null).toASCIIString();
				if (!file.isHidden() && !name.startsWith("."))
					f.format(" <a href=\"%s\">%s</a>%-" + (w - name.length()) +
									"s&#8206;%td-%<tb-%<tY %<tR%6s%n",
							link, name, "", file.lastModified(), size);
	...
	private static boolean acceptDebugFileName(String name) {
		if (name.startsWith("crash_") && name.endsWith(".txt")) return true;
		if (!name.endsWith(".png")) return false;
		
		if (name.startsWith("crash_")) return true;
		if (name.startsWith("early_error_")) return true;
		if (name.startsWith("pm_")) return true;
		if (name.startsWith("weekly_reward_")) return true;
